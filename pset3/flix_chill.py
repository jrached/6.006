...
class Movie:
    ...
    def __init__(self, id, date, rank):
        ...
        #Highest ranking value to the left and right of node
        self.hr = self
...
class FlixChillManager(AVL):
    ...
    def maintain(self):
        ...
        #Augment each node with the highest ranking movie between itself and its children
        if self.right is None and self.left is None:
            self.item.hr = self.item
        elif self.right is None:
            self.item.hr = max(self.item.hr, self.left.item.hr, key=lambda x: x.rank)
        elif self.left is None: 
            self.item.hr = max(self.item.hr, self.right.item.hr, key=lambda x: x.rank)
        else:
            self.item.hr = max(self.item.hr, self.right.item.hr, self.left.item.hr, key=lambda x: x.rank)
    ...
    def highest(self, s):
        ...
        # YOUR CODE HERE #
        ##################
        #Flag checks whether we have a right parent.
        right_parent = False

        #Find s.
        node = self.earliest(s)

        #Get the max value between s and the highest ranking movie to the right of s.
        if node.right is not None:
            max_so_far = max(node.item, node.right.item.hr, key=lambda x: x.rank)
        else:
            max_so_far = node.item
        
        #Go up the tree updating the highest ranking movie only if we go up through a right parent.
        while node is not None:
            
            if right_parent:
                if node.right is not None:
                    max_so_far = max(node.item, node.right.item.hr, max_so_far, key=lambda x: x.rank)
                else:
                    max_so_far = max(node.item, max_so_far, key=lambda x: x.rank)
            right_parent = node.parent is not None and node.parent.item.key > node.item.key
            node = node.parent
        return max_so_far.id
